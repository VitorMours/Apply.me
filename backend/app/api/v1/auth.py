...
async def signin(
    ...
    try:
        if finded_user := await user_service.fetch_user_by_email(data.email):
            raise UserFoundException(f"Existe registro com: {finded_user.email} dentro do banco de dados")
        
        user = await user_service.create_user(data, auth_service) 
        return user
